...
const offlineWalletRoutes = require('./routes/offlineWalletRoutes');
...
app.use('/api/wallet', offlineWalletRoutes);
...
app.use((err, req, res, next) => {
  ...
  try {
    const fs = require('fs');
    const logMsg = `[${new Date().toISOString()}] Global error handler error:\n${err.stack}\n\n`;
    fs.appendFileSync('error_log.txt', logMsg);
  } catch (e) {
    console.error('Failed to write to error_log.txt in global error handler:', e);
  }
  res.status(500).json({ 
    success: false, 

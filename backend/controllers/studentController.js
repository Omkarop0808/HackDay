...
// Helper to get or create a profile, handling parallel creation race conditions
const getOrCreateProfile = async (studentId) => {
  let profile = await StudentProfile.findOne({ student: studentId });
  if (!profile) {
    try {
      profile = new StudentProfile({ student: studentId });
      await profile.save();
    } catch (err) {
      if (err.code === 11000) {
        // Parallel request created it already, retrieve it
...
exports.getProfile = async (req, res) => {
  try {
    ...
    // Find profile or create it if not exists
    let profile = await getOrCreateProfile(student._id);
...
exports.updateProfile = async (req, res) => {
  try {
    ...
    if (req.body.aiPersonality) student.aiPersonality = req.body.aiPersonality;
    await student.save();
...
exports.updateGoal = async (req, res) => {
  try {
    ...
    const profile = await getOrCreateProfile(student._id);

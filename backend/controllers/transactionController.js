...
exports.getTransactionStats = async (req, res) => {
  try {
    ...
    // Deterministic Behavioral Meter Calculations
    // 1. Impulse Risk: Percentage of expenses in discretionary categories
    const discretionaryCategories = ['entertainment', 'shopping', 'dining_out', 'clothing', 'electronics'];
    const discretionaryTotal = categoryStats
      .filter(c => discretionaryCategories.includes(c._id))
      .reduce((sum, c) => sum + c.total, 0);
    
    let impulseRisk = 0;
    if (stats.totalExpense > 0) {
       impulseRisk = Math.round((discretionaryTotal / stats.totalExpense) * 100);
    }
    // Scale it slightly so it looks realistic (e.g. 20% discretionary = 40% risk)
    impulseRisk = Math.min(100, Math.max(5, impulseRisk * 1.5));

    // 2. Savings Stability: Based on positive savings rate and consistent income
    let savingsStability = 50; // baseline
    if (stats.savingsRate > 0) {
      savingsStability += stats.savingsRate;
    } else {
      savingsStability -= 20;
    }
    // Add stability if they have regular income (simplified check: transaction count > 0 for income)
    if (stats.totalIncome > 0) savingsStability += 10;
    savingsStability = Math.min(98, Math.max(10, savingsStability));

    // 3. Discipline: Overall metric based on budget adherence and low impulse risk
    // We don't have budget here directly, but we can use savingsRate and impulseRisk
    let discipline = 100 - impulseRisk;
    if (stats.savingsRate > 20) discipline += 10;
    else if (stats.savingsRate < 0) discipline -= 20;
    discipline = Math.min(99, Math.max(15, discipline));

    const behavioralMeter = {
      discipline: Math.round(discipline),
      impulseRisk: Math.round(impulseRisk),
      savingsStability: Math.round(savingsStability)
    };

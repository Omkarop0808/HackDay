...
exports.sendCopilotMessage = async (req, res) => {
  try {
    ...
    .sort({ date: -1 })
    .limit(50)
    .select('type amount category date merchant');
    ...
    const numericContext = {
      ...
      goals: {
        ...
        activeShortTermGoals: activeShortGoals.map(g => ({ title: g.title, targetAmount: g.targetAmount, currentAmount: g.currentAmount })),
        activeLongTermGoals: activeLongGoals.map(g => ({ title: g.title, targetAmount: g.targetAmount, currentAmount: g.currentAmount }))
      },
      investmentsAmount: profile?.investmentsAmount || 0,
    };

    const userContext = `CONTEXT_JSON:\n${JSON.stringify(numericContext)}`;

    // Mode-specific system prompts
    const modePrompts = {
      budget: 'You are a Budget Agent specializing in expense tracking and spending reduction. Help the user identify overspending, suggest cutbacks, point out wasted money in recent transactions, and optimize their budget allocation.',
  ...
  - You will receive financial data and a recent transaction history. Do not infer name, email, college, employer, addresses, or phone numbers.
  - You MAY reference specific transactions, merchants, and dates provided in the context to give highly tailored advice.
...
- Provide specific, actionable advice with exact numbers and merchant names from their data
  - Base advice strictly on the context
...
- Pay close attention to dates. Use the CURRENT DATE provided above to understand "yesterday", "last week", etc.
- Double-check all math step-by-step before answering.
- Income is money IN (+). Expenses are money OUT (-). Do not confuse them when calculating net cash flow.

FORMATTING RULES:
- Use Markdown to structure your response perfectly.
- Use **bold** text to highlight key numbers, merchant names, or important advice.
- Use bullet points (-) for listing transactions, tips, or steps.
- Break up large blocks of text with empty lines to create short, readable paragraphs.
- Use relevant emojis to make the response visually appealing and friendly.

...
import { DREAM_PLANNER_TIPS } from '../lib/retirement/tooltips';
import { calculateRetirement, formatIndian, INFLATION_RATE } from '../lib/retirement/math';
...
} from '../lib/retirement/lifestyleConfig.js';
...
const LifestyleCard = ({ type, selected, onClick, monthlyIncome, yearsToRetire }) => {
  ...
  return (
    <button
      ...
      className={`relative flex-1 bg-card border rounded-[16px] p-6 text-left transition-all duration-300 group ${
        selected
          ? 'border-[#8B5CF6] shadow-sm scale-[1.02]'
          : 'border-border shadow-sm hover:-translate-y-1 hover:shadow-sm'
      }`}
      type="button"
    >
      {config.badge && (
        <div className="absolute -top-3 -right-3 bg-[#FBBF24] border border-border px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest rotate-3 shadow-sm">
          {config.badge}
        </div>
      )}

      <div className="w-12 h-12 rounded-full flex items-center justify-center border border-border mb-4 shadow-sm" style={{ backgroundColor: `${config.accent}22` }}>
      ...
      <p className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-4">{config.subtitle}</p>
      ...
      <div className="pt-4 border-t border-slate-100 mb-4">
        <div className="text-[10px] font-black text-muted-foreground/80 uppercase tracking-widest mb-1">
      ...
      <div className="bg-background border border-border/10 rounded-full px-3 py-1 text-[9px] font-black uppercase tracking-widest text-center">
        {config.tag}
      </div>
    </button>
  );
  ...
  <div className="inline-flex bg-card border border-border rounded-full p-1 shadow-sm">
    <button
      ...
      className={`touch-target px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-[2px] transition-all ${
        mode === LIFESTYLE_MODES.PRESET ? 'bg-primary text-white' : 'text-muted-foreground'
      }`}
    >
      Preset
    </button>
    <button
      ...
      className={`touch-target px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-[2px] transition-all ${
        mode === LIFESTYLE_MODES.CUSTOM ? 'bg-primary text-white' : 'text-muted-foreground'
      }`}
    >
      Custom
    </button>
...
const CustomLifestyleEditor = ({ categoryAmounts, onCategoryAmountChange, monthlyIncome }) => {
  ...
  return (
    <div className="bg-card border border-border rounded-[20px] p-6 md:p-8 shadow-sm space-y-6">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-full border border-border bg-primary/10 flex items-center justify-center shadow-sm">
          <SlidersHorizontal className="w-5 h-5 text-primary" />
        </div>
        <div>
          <h3 className="font-heading font-black text-xl">Build Your Custom Monthly Lifestyle</h3>
          <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Set rupee amounts by category. We auto-calculate total spend.</p>
        </div>
      </div>

      <div className="rounded-xl border border-border bg-[#FFFDF5] p-4">
        <div className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/80">Monthly Spend (Auto)</div>
        <div className="font-heading font-black text-2xl text-foreground mt-1">{formatIndian(customMonthlySpend)}</div>
        <div className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/80 mt-1">Based on category amounts</div>
      </div>
      ...
      <div className="space-y-4">
        {CATEGORIES.map((cat) => {
          ...
          return (
            <div key={cat.id} className="border border-border/10 rounded-2xl p-4 space-y-3">
              <div className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-9 h-9 rounded-full border border-border flex items-center justify-center" style={{ backgroundColor: `${cat.color}22` }}>
                ...
                <div className="text-right">
                  <div className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/80">Share</div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full border border-border/20 rounded-lg pl-8 pr-3 py-2 text-sm font-black text-left focus:outline-none focus:shadow-sm"
                />
              </div>
            </div>
          );
      ...
      <div className={`text-[10px] font-black uppercase tracking-widest px-3 py-2 rounded-xl inline-flex flex-wrap gap-2 border border-border ${salaryOverdrawPct > 0 ? 'bg-red-100 text-red-700' : 'bg-emerald-100 text-emerald-700'}`}>
...
const InflationRealityCheck = ({ yearsToRetire, monthlySpendToday }) => {
  ...
  return (
    <div className="bg-[#FBBF24] border border-border rounded-[24px] p-8 shadow-sm flex flex-col md:flex-row items-center justify-between gap-8">
      <div className="space-y-2 text-center md:text-left">
        <div className="text-xs font-black uppercase tracking-[3px] text-foreground">
        ...
        <p className="text-sm md:text-lg font-bold text-muted-foreground uppercase tracking-widest">
          ...will cost {formatIndian(monthlySpendRetirement)}/month at retirement.
        </p>
        <div className="text-[10px] font-bold text-muted-foreground/80 uppercase tracking-widest pt-2">
          Assuming 6% inflation over {yearsToRetire} years
        </div>
      </div>

      <div className="text-center group">
        <div className="font-heading font-black text-7xl md:text-8xl text-foreground leading-none transition-transform group-hover:scale-110 duration-500">
          {displayMultiplier.toFixed(1)}x
        </div>
        <div className="text-xs font-black uppercase tracking-widest text-muted-foreground mt-2">
  ...
  <div className="space-y-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {CATEGORIES.map((cat) => {
        ...
        return (
          <div key={cat.id} className="bg-card border border-border rounded-[16px] p-5 shadow-sm group hover:-translate-y-1 transition-all">
            <div className="flex items-center gap-4 mb-4">
              <div className="w-10 h-10 rounded-full flex items-center justify-center border border-border shadow-sm" style={{ backgroundColor: `${cat.color}22` }}>
                <Icon className="w-5 h-5 text-foreground" strokeWidth={2.5} style={{ color: cat.color }} />
              </div>
              <div className="font-bold text-sm text-foreground uppercase tracking-widest flex items-center gap-1">
                {cat.name} <InfoTooltip text={DREAM_PLANNER_TIPS[cat.tooltipKey] || DREAM_PLANNER_TIPS.categoryHousing} size={12} />
              </div>
            </div>
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="text-center">
                <div className="text-[9px] font-black text-muted-foreground/60 uppercase tracking-widest mb-1">Today</div>
                <div className="font-bold text-sm tracking-tight">{formatIndian(today)}</div>
              </div>
              <ArrowRight className="w-4 h-4 text-slate-200" />
              <div className="text-center">
                <div className="text-[9px] font-black text-muted-foreground/60 uppercase tracking-widest mb-1">Retirement</div>
            ...
            <div className="mt-2 text-[9px] font-black text-muted-foreground/80 uppercase tracking-widest text-right">
...
const PageContent = () => {
  const { student: userData } = useUser();
  ...
  const monthlyIncome = Math.max(50000, Number(userData?.monthlyIncome) || 0);
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      // Mock save to backend API
      await new Promise(r => setTimeout(r, 800));
      toast.success("Dream updated! Your dashboard reflects your new plan.");
    } catch (error) {
      console.error(error);
    } finally {
      setIsSaving(false);
    ...
    <div className="p-3 sm:p-4 md:p-8 space-y-10 sm:space-y-12 max-w-6xl mx-auto pb-[calc(13rem+env(safe-area-inset-bottom))] lg:pb-32">
      <section className="space-y-6">
        <div className="text-center md:text-left space-y-2">
          ...
          <p className="text-sm md:text-base font-bold text-muted-foreground uppercase tracking-widest">Choose a preset or tune every bucket yourself.</p>
        </div>
      ...
      <section className="bg-card border border-border rounded-[24px] overflow-hidden shadow-sm">
        <div className="w-1.5 absolute left-0 top-0 h-full bg-primary" />
        <div className="p-8 flex flex-col md:flex-row items-center justify-between gap-12">
          <div className="flex-1 space-y-8 w-full">
            <div className="grid grid-cols-2 gap-8 items-center">
              <div className="space-y-4 text-center">
                <div className="text-[10px] font-black text-muted-foreground/80 uppercase tracking-[2px]">CURRENT</div>
                <div className="flex flex-col items-center">
                  <div className="w-20 h-20 rounded-full border-4 border-border flex items-center justify-center font-heading font-black text-2xl text-slate-400 bg-background">
                    {formatScore(baseScore)}
                  </div>
                  <div className="mt-2 text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full bg-muted text-slate-500 border border-border">
              ...
              <div className="space-y-4 text-center relative">
                ...
                <div className="text-[10px] font-black text-primary uppercase tracking-[2px]">WITH THIS PLAN</div>
                <div className="flex flex-col items-center">
                  <div
                    className="w-24 h-24 rounded-full border-4 flex items-center justify-center font-heading font-black text-3xl transition-all duration-500 shadow-sm"
                    style={{ borderColor: scoreInfo(simScore).color, color: scoreInfo(simScore).color }}
                  >
                    {formatScore(simScore)}
                  </div>
                  <div className="flex items-center gap-2 mt-2">
                    <span className="text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full border border-border" style={{ backgroundColor: `${COLORS.violet}22`, color: COLORS.violet }}>
                      {planMode === LIFESTYLE_MODES.CUSTOM ? 'custom' : selectedLifestyle}
                    </span>
                    {(scoreDelta !== 0 || cappedButMoved) && (
                      <span className={`text-[9px] font-black px-2 py-0.5 rounded-full border border-border ${scoreDelta > 0 ? 'bg-emerald-100 text-emerald-600' : 'bg-red-100 text-red-600'}`}>
                        {scoreDelta !== 0
            ...
            <div
              className={`p-4 rounded-xl border border-border font-bold text-xs uppercase tracking-widest flex items-center gap-3 ${
                scoreDelta > 0
          ...
          <div className="hidden lg:block w-[300px] h-[300px] relative">
            <div className="absolute inset-0 bg-primary/5 rounded-full border border-dashed border-[#8B5CF6]/20 animate-spin-slow" />
            <div className="absolute inset-10 bg-card border border-border rounded-[32px] shadow-sm flex items-center justify-center">
      ...
      <div className="fixed bottom-[calc(4.5rem+env(safe-area-inset-bottom))] lg:bottom-0 left-0 dashboard-fixed-offset right-0 min-h-[4.75rem] lg:h-20 bg-card border-t-2 border-border z-40 px-3 sm:px-4 lg:px-6 py-2 sm:py-3 lg:py-0 flex flex-wrap sm:flex-nowrap items-center justify-between gap-3 shadow-sm">
        ...
        <div className="flex items-center gap-4">
          <button
            ...
            onClick={handleReset}
            disabled={!hasChanges}
            className="touch-target px-4 sm:px-6 py-2.5 rounded-full font-black text-[10px] sm:text-xs uppercase tracking-widest text-slate-400 enabled:hover:text-foreground transition-colors flex items-center gap-2 disabled:opacity-40"
          >
          ...
          <button
            ...
            className="touch-target px-5 sm:px-8 py-3 bg-primary border border-border rounded-full shadow-sm enabled:hover:-translate-y-0.5 enabled:active:translate-y-0.5 enabled:active:shadow-sm transition-all text-white text-[10px] sm:text-xs font-black uppercase tracking-[2px] flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isSaving ? 'Saving...' : (
              <>
...
export default function RetirementPlanner() {
  return (
    <div className="w-full min-h-screen bg-background font-sans pb-24 md:pb-6">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 bg-card/80 backdrop-blur-md border-b-2 border-border shadow-sm md:static sticky top-0 z-40 mb-6">
        <div>
          <h1 className="text-xl md:text-2xl font-heading font-black text-foreground tracking-tight">Dream Planner</h1>
          <p className="text-xs font-bold text-slate-500 uppercase tracking-wider mt-1">Design your lifestyle & hit the target</p>
        </div>
      </div>
      <PageContent />
    </div>

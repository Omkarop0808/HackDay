...
function DebtScene({ data, colors }) {
  const totalDebt = parseInt(data?.totalDebt, 10) || 0;
  const monthlyPayment = parseInt(data?.monthlyPayment, 10) || 0;
        ...
        <div style={{ marginBottom: 24 }}>
          {debts.slice(0, 3).map((debt, i) => (
            <AnimatedBar
              ...
              label={debt.name}
              value={debt.balance}
              maxValue={Math.max(...debts.map((d) => parseInt(d.balance, 10) || 0), 1)}
              delay={20 + i * 10}
              color={colors.warning}
              suffix="Rs."
              colors={colors}
